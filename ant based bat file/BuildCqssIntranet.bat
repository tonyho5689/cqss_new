...
@if GO%1==GOSKIP GOTO CQSS_INTRANET
...
SET BUILD_LOG_FILEPATH=.\BuildCqssIntranet.log
SET BUILD_LOG_FILEPATH_STR= ^>^> %BUILD_LOG_FILEPATH% 2^>^>&1
@if exist %BUILD_LOG_FILEPATH% del %BUILD_LOG_FILEPATH%

@echo Run BuildCqssIntranet Starting: %date% %time% %BUILD_LOG_FILEPATH_STR%
...
@Call ant -Dwashome="%WAS_HOME%" -Dcbslibroot="../../../cbs/cbs/Deploy/Was9/Libraries" -buildfile BuildCqssIntranet.xml %BUILD_LOG_FILEPATH_STR%

@if not GO%1==GOSKIP (
    @echo Run BuildCqssIntranet Ending: %date% %time% %BUILD_LOG_FILEPATH_STR%
)
