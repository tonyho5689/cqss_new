...
@if GO%1==GOSKIP GOTO CQSS_APPLET
...
SET BUILD_LOG_FILEPATH=.\BuildCqssForClient.log
SET BUILD_LOG_FILEPATH_STR= ^>^> %BUILD_LOG_FILEPATH% 2^>^>&1
@if exist %BUILD_LOG_FILEPATH% del %BUILD_LOG_FILEPATH%

@echo Run BuildCqssForClient Starting: %date% %time% %BUILD_LOG_FILEPATH_STR%

:CQSS_APPLET
if not exist %BUILD_LOG_FILEPATH% SET BUILD_LOG_FILEPATH_STR=
@Call CleanupFolder.bat %1 %BUILD_LOG_FILEPATH_STR%
@Call SetupEnv_jdk1.8.bat %1 %BUILD_LOG_FILEPATH_STR%

@SET PATH=%ANT_HOME%\bin;%PATH%
@Call ant -Dwashome="%WAS_HOME%" -buildfile BuildCqssForClient.xml %BUILD_LOG_FILEPATH_STR%

@if not GO%1==GOSKIP (
    @echo Run BuildCqssForClient Ending: %date% %time% %BUILD_LOG_FILEPATH_STR%
)
